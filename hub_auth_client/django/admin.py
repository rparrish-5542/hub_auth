        ...
        custom_urls = [
            path(
                ...
                self.admin_site.admin_view(
                    self.sync_azure_scopes_view),
                name='sync_azure_scopes'),
        ]
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                'discover-unsecured/',
                self.admin_site.admin_view(
                    self.discover_unsecured_view),
                name='discover_unsecured_endpoints'),
            path(
                'create-permissions/',
                self.admin_site.admin_view(
                    self.create_endpoint_permissions_view),
                name='create_endpoint_permissions'),
        ]
        return custom_urls + urls
    ...
    def create_endpoint_permissions_view(self, request):
        ...
        for endpoint_data in selected_endpoints:
            try:
                ...
                url_parts = [
                    p for p in url_pattern.replace(
                        '^', '').replace(
                        '$', '').split('/') if p and not p.startswith('?P<')]
        ...
        def get_urls(self):
            ...
            custom_urls = [
                path(
                    'discover-tables/',
                    self.admin_site.admin_view(
                        self.discover_tables_view),
                    name='discover_rls_tables'),
                path(
                    'create-table-configs/',
                    self.admin_site.admin_view(
                        self.create_table_configs_view),
                    name='create_table_configs'),
            ]
            return custom_urls + urls
